...
def _build_engines(modes: list[str]) -> dict[str, AsyncEngine]:
    """Instantiate one engine per mode (all share the same MusicUseCase)."""
    engines: dict[str, AsyncEngine] = {}
    for mode in modes:
        if mode == "sync":
            engines[mode] = SyncEngine(MusicUseCase)
        else:
            injection = mode.split("/")[1]   # "tool" | "system" | "user"
            engines[mode] = AsyncEngine(MusicUseCase, injection_mode=injection)
    return engines
...
def run_benchmark(args: argparse.Namespace) -> list[ScenarioBenchmarkResult]:
    ...
    if args.tool_delay is not None:
        ...
        sleep_patch = mock.patch(
            ...
            side_effect=lambda _orig: __import__("time").sleep(delay),
        )
        sleep_patch.start()
...
def main() -> None:
    ...
    print(f"  tool-delay : {args.tool_delay if args.tool_delay is not None else 'real (2–4s)'}")
